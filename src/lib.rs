...
macro_rules! fn_compose {
    ...
    ( $arg: expr => $f: ident $( $largs: expr, )* ; $( ,$rargs: expr )* ) => {
        $f($($largs,)* $arg $(,$rargs)*)
    };
    ( $arg: expr => $f: ident => $( $tokens: tt )* ) => {{
        fn_compose!($f($arg) => $($tokens)*)
    }};
    ( $arg: expr => $f: ident $( $largs: expr, )* ; $( ,$rargs: expr )* => $( $tokens: tt )* ) => {{
...
mod tests {
    ...
    fn one_arg_fn() {
        let add = |x, y| x + y;
        assert_eq!(fn_compose!(4 => add 5, ;), 9);
    }
    ...
    fn two_arg_fn() {
        ...
        let add = |a, b, c| a + b + c;
        assert_eq!(fn_compose!(4 => add 3, ;, 9 => mul 1, 4, 2, ;, 10), 1280);
    }
}
